...
show_ssl_hint() {
    ...
    echo ""
    echo "2. 获取 SSL 证书 (首次):"
    echo "   sudo certbot certonly --standalone -d huauge.com -d www.huauge.com --email admin@huauge.com --agree-tos"
    echo ""
    echo "3. 设置自动续费 hook:"
    echo "   sudo mkdir -p /etc/letsencrypt/renewal-hooks/post"
    echo "   echo '#!/bin/bash' | sudo tee /etc/letsencrypt/renewal-hooks/post/reload-nginx.sh"
    echo "   echo 'docker exec bycigar-frontend nginx -s reload' | sudo tee -a /etc/letsencrypt/renewal-hooks/post/reload-nginx.sh"
    echo "   sudo chmod +x /etc/letsencrypt/renewal-hooks/post/reload-nginx.sh"
    echo ""
    echo "4. 测试续费:"
